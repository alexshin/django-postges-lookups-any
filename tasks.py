...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-postges-lookups-any runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-postges-lookups-any.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_postges_lookups_any")
...
def lint(c):
    ...
    """
    c.run("flake8 django-postges-lookups-any tests")
...
def release(c, bumpsize=''):
    ...
    import django_postges_lookups_any
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_postges_lookups_any.__version__))
    c.run("git push --tags")
